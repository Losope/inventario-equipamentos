...
function InventarioEquipamentos() {
  ...
  const dashboard = useMemo(() => {
    ...
    const porTipo = TIPOS.map((t) => {
      ...
      return { tipo: t.key, emoji: t.emoji, total: doTipo.length, conferidos: conf };
    });
    const porPessoa = {};
    ...
    return { totalEquip, conferidos, porTipo, ranking, recentes, totalConferencias: conferencias.length };
  }, [items, conferencias]);
  ...
  return (
    <div style={{ background: "#12161A", minHeight: "100vh", color: "#E8EDF2", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 20px 60px" }}>
        ...
        {dashboardOpen && (
          <div style={{ background: "#1B2128", border: "1px solid #2E3742", padding: 20, marginBottom: 20 }}>
            <h2 className="disp" style={{ fontSize: 20, fontWeight: 800, textTransform: "uppercase", margin: "0 0 16px" }}>📊 Dashboard geral</h2>
            ...
            <div style={{ marginBottom: 22 }}>
              <div style={{ fontSize: 12, color: "#8A94A0", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 10 }}>Cobertura de conferência por tipo</div>
              {dashboard.porTipo.map((t) => (
                <div key={t.tipo} style={{ marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, marginBottom: 4 }}>
                    <span>{t.emoji} {t.tipo}</span>
                    <span className="mono">{t.conferidos}/{t.total}</span>
                  </div>
                  <div style={{ background: "#12161A", height: 6 }}>
                    <div style={{ background: "#3DD68C", height: 6, width: t.total ? `${(t.conferidos / t.total) * 100}%` : "0%" }} />
                  </div>
                </div>
              ))}
            </div>
