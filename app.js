...
function dadosRotuloParaCadastro(dados) {
  ...
  const modelo = [dados["Marca/Modelo"], dados["Numero de serie"] ? `SN ${dados["Numero de serie"]}` : null].filter(Boolean).join(" · ");
  const responsaveis = {
  ...
  return { codigo, tipo, modelo, setor: dados["Area"] || "", responsaveis };
}
...
function BarcodeModal({ target, items, pessoas, onAddPessoa, onPreCadastro, onClose, onConfirm }) {
  ...
  const preCadastrarComRotulo = () => {
    if (!dadosRotulo) return;
    const novo = onPreCadastro(dadosRotulo.codigo, dadosRotulo.tipo, dadosRotulo);
            ...
            {!target && !selectedItem && dadosRotulo && (
              <div style={{ background: "#1F1B2E", border: "1px solid #B26EF5", padding: 12, marginBottom: 14 }}>
                <div style={{ fontSize: 13, color: "#D8AFFA", marginBottom: 10 }}>
                  Dados completos identificados no rótulo — não precisa preencher nada, já vem tudo pronto:
                </div>
                <div style={{ fontSize: 12.5, color: "#E8EDF2", lineHeight: 1.7, marginBottom: 10 }}>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><TipoIcone tipoKey={dadosRotulo.tipo} size={13} color="#D8AFFA" /> {dadosRotulo.tipo}</span>
                  {dadosRotulo.modelo && <> · {dadosRotulo.modelo}</>}
                  <br />Área: {dadosRotulo.setor || "—"}
                  <br />Responsáveis: {resumoResponsaveis(dadosRotulo.responsaveis)}
                </div>
                <button onClick={preCadastrarComRotulo} style={{ ...primaryBtn, width: "100%", justifyContent: "center", background: "#B26EF5", color: "#1F1B2E" }}>
                  🆕 Pré-cadastrar com esses dados
                </button>
              </div>
            )}
