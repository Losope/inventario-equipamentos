...
function mapTipoTexto(t) {
  const s = (t || "").toLowerCase();
  if (s.includes("radio") || s.includes("rádio")) return "Rádio";
  if (s.includes("coletor")) return "Coletor";
  return "Tablet";
}

function parseQRTexto(raw) {
  if (!raw) return null;
  const texto = raw.replace(/\s+/g, " ").trim();
  const dados = {};
  CAMPOS_ROTULO_QR.forEach((campo, i) => {
    const restantes = CAMPOS_ROTULO_QR.slice(i + 1).map(escapeRegex).join("|");
    const fim = restantes ? `(?:${restantes})\\s*:|$` : "$";
    const re = new RegExp(`${escapeRegex(campo)}\\s*:\\s*(.*?)\\s*(?=${fim})`, "i");
    const m = texto.match(re);
    if (m && m[1]) dados[campo] = m[1].trim();
  });
  if (!dados["ID"] && !dados["Identificacao"]) return null;
  return dados;
}

function dadosRotuloParaCadastro(dados) {
  const codigo = dados["ID"] || dados["Identificacao"];
  const tipo = mapTipoTexto(dados["Tipo"]);
  const responsaveis = {
    T1: dados["Responsavel T1"] ? [dados["Responsavel T1"]] : [],
    T2: dados["Responsavel T2"] ? [dados["Responsavel T2"]] : [],
    T3: dados["Responsavel T3"] ? [dados["Responsavel T3"]] : [],
    Escalas: [],
  };
  return {
    codigo,
    tipo,
    numeroEquipamento: dados["Numero do equipamento"] || "",
    marcaModelo: dados["Marca/Modelo"] || "",
    numeroSerie: dados["Numero de serie"] || "",
    setor: dados["Area"] || "",
    responsaveis,
  };
}
...
function InventarioEquipamentos() {
  ...
  const preCadastrarPorCodigo = (codigo, tipo, extra) => {
    const now = new Date().toISOString();
    const responsaveis = (extra && extra.responsaveis) || emptyResponsaveis();
    const temResp = hasResponsavel(responsaveis);
    const novo = {
      id: uid(),
      tipo,
      patrimonio: codigo,
      modelo: (extra && extra.modelo) || "",
      numeroEquipamento: (extra && extra.numeroEquipamento) || "",
      numeroSerie: (extra && extra.numeroSerie) || "",
      setor: (extra && extra.setor) || "",
      responsaveis,
      status: temResp ? "Em uso" : extra ? "Disponível" : "Pendente",
      atualizado: now,
      conferido: false,
      conferidoEm: null,
      conferidoPor: null,
      historico: [
        {
          data: now,
          evento: extra ? "Pré-cadastro (dados do rótulo QR)" : "Pré-cadastro (QR/código)",
          responsaveis: resumoResponsaveis(responsaveis),
          setor: (extra && extra.setor) || "—",
          obs: extra
            ? "Criado automaticamente a partir dos dados completos lidos do rótulo (colados via texto do QR)."
            : "Criado automaticamente pela leitura do código. Área e responsável ainda pendentes — complete em Movimentar.",
        },
      ],
    };
    persist([novo, ...items]);
  ...
  return (
    <div style={{ background: "#12161A", minHeight: "100vh", color: "#E8EDF2", fontFamily: "'Inter', sans-serif" }}>
      ...
      {checkModal && (
        <BarcodeModal target={checkModal.item} items={items} pessoas={pessoas} areas={areasConhecidas} onAddPessoa={addPessoa} onPreCadastro={preCadastrarPorCodigo} onClose={() => setCheckModal(null)} onConfirm={confirmConferencia} />
      )}
    </div>
  );
}
...
const miniLabel = { fontSize: 10.5, color: "#8A94A0", marginBottom: 3, display: "block", textTransform: "uppercase", letterSpacing: 0.4 };
const miniField = { width: "100%", background: "#12161A", border: "1px solid #2E3742", color: "#E8EDF2", padding: "7px 8px", fontSize: 12.5 };
...
function BarcodeModal({ target, items, pessoas, areas, onAddPessoa, onPreCadastro, onClose, onConfirm }) {
  ...
  const [dadosRotulo, setDadosRotulo] = useState(null); // dados completos extraídos do texto do rótulo
  const [formTipo, setFormTipo] = useState("Tablet");
  const [formId, setFormId] = useState("");
  const [formNumEquip, setFormNumEquip] = useState("");
  const [formModelo, setFormModelo] = useState("");
  const [formSerie, setFormSerie] = useState("");
  const [formSetor, setFormSetor] = useState("");
  const [formResponsaveis, setFormResponsaveis] = useState(emptyResponsaveis());
  ...
  const processarTextoColado = () => {
    const dados = parseQRTexto(textoColado);
    if (!dados) {
      setError("Não reconheci um rótulo válido nesse texto. Confira se colou o conteúdo completo lido pela câmera do celular.");
      return;
    }
    setError("");
    const cadastro = dadosRotuloParaCadastro(dados);
    setDadosRotulo(cadastro);
    setFormTipo(cadastro.tipo);
    setFormId(cadastro.codigo);
    setFormNumEquip(cadastro.numeroEquipamento);
    setFormModelo(cadastro.marcaModelo);
    setFormSerie(cadastro.numeroSerie);
    setFormSetor(cadastro.setor);
    setFormResponsaveis(cadastro.responsaveis);
    setColar(false);
    setTextoColado("");
    aplicarCodigo(cadastro.codigo);
  };
  ...
  const lerPorFoto = async (e) => {
    ...
    if ("BarcodeDetector" in window) {
      try {
        ...
        const detector = new BarcodeDetector({
          formats: ["qr_code", "code_128", "code_39", "code_93", "ean_13", "ean_8", "upc_a", "upc_e", "codabar", "itf", "data_matrix", "pdf417", "aztec"],
        });
        const resultados = await detector.detect(bitmap);
        if (resultados && resultados.length) texto = resultados[0].rawValue;
      } catch {}
    }
    ...
    if (!texto) {
      try {
        const html5Qr = new Html5Qrcode(regionId);
        texto = await html5Qr.scanFile(file, false);
        try {
          html5Qr.clear();
        } catch {}
      } catch {}
    ...
    if (texto) {
      ...
      setError("Não foi possível identificar um código nessa foto. Esse código pode estar denso/borrado demais para leitura por navegador — vale considerar reimprimir com um tamanho maior, ou usar a leitura manual por enquanto.");
    }
  ...
  const lerProximo = () => {
    ...
    setFormSerie("");
    setFormSetor("");
    setFormResponsaveis(emptyResponsaveis());
    setColar(false);
    setTextoColado("");
    setError("");
    iniciarScanner();
  ...
  const preCadastrar = () => {
    ...
    setConfirmado({ patrimonio: novo.patrimonio, preCadastro: true, completo: false });
    setResult(null);
  };

  const preCadastrarComRotulo = () => {
    if (!formId.trim()) return;
    const modeloCombinado = [formModelo.trim(), formSerie.trim() ? `SN ${formSerie.trim()}` : null].filter(Boolean).join(" · ");
    const extra = {
      modelo: modeloCombinado,
      numeroEquipamento: formNumEquip.trim(),
      numeroSerie: formSerie.trim(),
      setor: formSetor.trim(),
      responsaveis: formResponsaveis,
    };
    const novo = onPreCadastro(formId.trim(), formTipo, extra);
    setConfirmado({ patrimonio: novo.patrimonio, preCadastro: true, completo: true });
    setResult(null);
    setDadosRotulo(null);
  };
    ...
    <div style={overlay} onClick={onClose}>
      <div style={{ ...modal, maxWidth: 560 }} onClick={(e) => e.stopPropagation()}>
          ...
          <div style={{ marginBottom: 4 }}>
            {confirmado.preCadastro ? (
              <div style={{ background: "#241A2E", border: "1px solid #B26EF5", color: "#D8AFFA", padding: "10px 12px", fontSize: 13, marginBottom: 14 }}>
                🆕 <span className="mono">{confirmado.patrimonio}</span>{" "}
                {confirmado.completo
                  ? "pré-cadastrado com os dados completos do rótulo (tipo, área e responsáveis já preenchidos)."
                  : (<>pré-cadastrado como <strong>Pendente</strong>. Área e responsável ficam pendentes — complete depois em "Movimentar".</>)}
              </div>
            ) : (
        ...
        {!confirmado && !result && !manual && (
          <>
            <div id={regionId} style={{ width: "100%", height: scanning ? 420 : 0, overflow: "hidden", background: "#000", marginBottom: scanning ? 12 : 0 }}></div>
            {scanning && zoomCap && (
            ...
            <button onClick={() => fotoInputRef.current?.click()} disabled={analisandoFoto} style={{ ...iconBtn, width: "100%", justifyContent: "center", marginBottom: 8, fontSize: 12.5, opacity: analisandoFoto ? 0.6 : 1 }}>
              {analisandoFoto ? "Analisando foto…" : "📸 Tirar foto para ler (códigos difíceis)"}
            </button>
            <input ref={fotoInputRef} type="file" accept="image/*" capture="environment" onChange={lerPorFoto} style={{ display: "none" }} />
            <button onClick={() => setColar(true)} style={{ ...iconBtn, width: "100%", justifyContent: "center", marginBottom: 8, fontSize: 12.5 }}>
              📋 Colar texto lido pela câmera do celular
            </button>
        ...
        {!confirmado && colar && !result && (
          <>
            <p style={{ fontSize: 12.5, color: "#8A94A0", marginTop: 0, marginBottom: 10 }}>
              Se o código for muito denso pra câmera do app ler, use a câmera nativa do celular (a de tirar foto normal) pra ler o QR, copie o texto que aparecer, e cole aqui embaixo.
            </p>
            <label style={label}>Texto lido do rótulo</label>
            <textarea style={{ ...field, minHeight: 90, resize: "vertical", fontSize: 12.5 }} autoFocus value={textoColado} onChange={(e) => setTextoColado(e.target.value)}
              placeholder="Cole aqui o texto completo lido pela câmera nativa" />
            <button disabled={!textoColado.trim()} onClick={processarTextoColado}
              style={{ ...primaryBtn, width: "100%", justifyContent: "center", marginBottom: 8, opacity: textoColado.trim() ? 1 : 0.5 }}>
              Processar texto
            </button>
            <button onClick={() => { setColar(false); setTextoColado(""); }} style={{ ...iconBtn, width: "100%", justifyContent: "center", marginBottom: 14, fontSize: 12.5 }}>
              Voltar para leitura por câmera
            </button>
          </>
        )}
        ...
        {!confirmado && result && (
          <div style={{ marginBottom: 4 }}>
            ...
            {!target && !selectedItem && dadosRotulo && (
              <div style={{ background: "#1F1B2E", border: "1px solid #B26EF5", padding: 12, marginBottom: 14 }}>
                <div style={{ fontSize: 13, color: "#D8AFFA", marginBottom: 10 }}>
                  Dados identificados no rótulo — confira e ajuste antes de concluir:
                </div>

                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
                  <div>
                    <label style={miniLabel}>Tipo</label>
                    <select style={miniField} value={formTipo} onChange={(e) => setFormTipo(e.target.value)}>
                      {TIPOS.map((t) => <option key={t.key} value={t.key}>{t.key}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={miniLabel}>ID</label>
                    <input style={miniField} value={formId} onChange={(e) => setFormId(e.target.value)} />
                  </div>
                  <div>
                    <label style={miniLabel}>Número do equipamento</label>
                    <input style={miniField} value={formNumEquip} onChange={(e) => setFormNumEquip(e.target.value)} />
                  </div>
                  <div>
                    <label style={miniLabel}>Marca/Modelo</label>
                    <input style={miniField} value={formModelo} onChange={(e) => setFormModelo(e.target.value)} />
                  </div>
                  <div style={{ gridColumn: "1 / -1" }}>
                    <label style={miniLabel}>Número de série</label>
                    <input style={miniField} value={formSerie} onChange={(e) => setFormSerie(e.target.value)} />
                  </div>
                </div>

                <label style={label}>Área</label>
                <input style={field} list="lista-areas-conhecidas" value={formSetor} onChange={(e) => setFormSetor(e.target.value)} placeholder="Selecione ou digite a área" />
                <datalist id="lista-areas-conhecidas">
                  {(areas || []).map((a) => <option key={a} value={a} />)}
                </datalist>

                <ResponsaveisPorTurno value={formResponsaveis} onChange={setFormResponsaveis} pessoas={pessoas} />

                <button onClick={preCadastrarComRotulo} disabled={!formId.trim()}
                  style={{ ...primaryBtn, width: "100%", justifyContent: "center", background: "#B26EF5", color: "#1F1B2E", opacity: formId.trim() ? 1 : 0.5, cursor: formId.trim() ? "pointer" : "not-allowed" }}>
