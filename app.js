...
function emptyResponsaveis() {
  return { T1: "", T2: "", T3: "", Escalas: { escala: "", nome: "" } };
}

function hasResponsavel(r) {
  return !!(r && (r.T1 || r.T2 || r.T3 || (r.Escalas && r.Escalas.nome)));
}

function resumoResponsaveis(r) {
  if (!r) return "—";
  const partes = [];
  if (r.T1) partes.push(`T1: ${r.T1}`);
  if (r.T2) partes.push(`T2: ${r.T2}`);
  if (r.T3) partes.push(`T3: ${r.T3}`);
  if (r.Escalas && r.Escalas.nome) partes.push(`${r.Escalas.escala || "Escala"}: ${r.Escalas.nome}`);
...
function InventarioEquipamentos() {
  ...
  const filtered = useMemo(() => {
    return items.filter((it) => {
      ...
      const respText = [it.responsaveis?.T1, it.responsaveis?.T2, it.responsaveis?.T3, it.responsaveis?.Escalas?.nome].filter(Boolean).join(" ").toLowerCase();
    ...
    <div style={{ background: "#12161A", minHeight: "100vh", color: "#E8EDF2", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 20px 60px" }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginBottom: 16 }}>
          ...
          <button onClick={() => setPwOpen(true)} style={primaryBtn}>➕ Cadastrar equipamento</button>
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {filtered.map((it) => {
              const tipoInfo = TIPOS.find((t) => t.key === it.tipo);
              return (
                <div key={it.id} style={{ background: "#1B2128", border: "1px solid #2E3742", borderLeft: `3px solid ${statusColor(it.status)}`, padding: "14px 18px", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 20, flexShrink: 0 }}>{tipoInfo?.emoji}</span>
      ...
      {pwOpen && <PasswordModal onClose={() => setPwOpen(false)} onSuccess={() => { setPwOpen(false); setAddOpen(true); }} />}
...
function ResponsaveisPorTurno({ value, onChange }) {
  ...
  const setSimples = (tab, nome) => onChange({ ...value, [tab]: nome });
  const setEscala = (campo, v) => onChange({ ...value, Escalas: { ...value.Escalas, [campo]: v } });
  const preenchido = {
    T1: !!value.T1, T2: !!value.T2, T3: !!value.T3,
    Escalas: !!(value.Escalas && value.Escalas.nome),
  };

  return (
    <div style={{ marginBottom: 14 }}>
      <label style={label}>Responsável por turno (até 4 pessoas)</label>
      ...
      {activeTab === "Escalas" ? (
        <>
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 8 }}>
            {ESCALAS.map((esc) => (
              <button key={esc} type="button" onClick={() => setEscala("escala", esc)}
                style={{ padding: "6px 10px", fontSize: 12,
                  background: value.Escalas.escala === esc ? "#FF7A1A" : "#1B2128",
                  color: value.Escalas.escala === esc ? "#12161A" : "#8A94A0",
                  border: "1px solid #2E3742", cursor: "pointer" }}>
                {esc}
              </button>
            ))}
          </div>
          <input style={{ ...field, marginBottom: 4 }} value={value.Escalas.nome} onChange={(e) => setEscala("nome", e.target.value)}
            placeholder={value.Escalas.escala ? `Responsável (${value.Escalas.escala})` : "Selecione a escala e informe o nome"} />
        </>
      ) : (
        <input style={{ ...field, marginBottom: 4 }} value={value[activeTab]} onChange={(e) => setSimples(activeTab, e.target.value)}
          placeholder={`Responsável do turno ${activeTab}`} />
      )}
      <div className="mono" style={{ fontSize: 11, color: "#5F6A75" }}>{resumoResponsaveis(value)}</div>
    </div>
  );
}
  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={{ ...modal, maxWidth: 340 }} onClick={(e) => e.stopPropagation()}>
        ...
        <p style={{ fontSize: 13, color: "#8A94A0", marginTop: 0, marginBottom: 14 }}>Digite a senha para cadastrar um novo equipamento.</p>
...
function BarcodeModal({ target, items, pessoas, onAddPessoa, onClose, onConfirm }) {
  ...
  const iniciarScanner = async () => {
    ...
    try {
      ...
      setScanning(false);
      setError("Não foi possível acessar a câmera. Verifique a permissão do navegador ou use a leitura manual.");
    }
  };
  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={modal} onClick={(e) => e.stopPropagation()}>
        ...
        {!result && !manual && (
          <>
            <div id={regionId} style={{ width: "100%", minHeight: scanning ? 220 : 0, background: "#000", marginBottom: 12 }}></div>
            {!scanning ? (
        ...
        {result && (
          <div style={{ marginBottom: 4 }}>
            ...
            <button
              ...
              onClick={() => { if (novaPessoa && conferente.trim()) onAddPessoa(conferente.trim()); onConfirm(selectedItem, result.codigo, conferente.trim()); }}
